...
/*:
 * @plugindesc (v.1.2.1)[PRO] Crafting
 * @author Pheonix KageDesu
 * @target MZ MV
 * @url https://kdworkshop.net/plugins/simple-crafting
 *
 * @help
 * ---------------------------------------------------------------------------
 * 
 ...
 *      (recipe item - item with <craftAutoRecipe:X> notetag)
 *      More informaton about this notetag see below
 *
 ...
 *  You can use multiple <craft:....> notetags per Item (it's allows Item have many recipes)
 *
 ...
 *
 *
 * - <craftFreePositions> - if you add this notetag, positions of materials not matter for crafting
 *
 *
 * - <craftChance:X> - chance (from 1 to 100%) to success craft item. If crafting is failed, you lose materials anyway
 *      (Don't forget set Plugin Parameter "Is Use Chance System?" to TRUE)
 *  You also can write some formulas and script
 *
 * Example: <craftChance:25> - 25%
            <craftChance:$gameVariables.value(33)> - Value % from Variable 33
 *
 * ---------------------------------------------------------------------------
 ...
 *
 * @requiredAssets img/pSimpleCrafting/helpWindowStats
* @requiredAssets img/pSimpleCrafting/windowCloseButton_01
* @requiredAssets img/pSimpleCrafting/windowCloseButton_00
* @requiredAssets img/pSimpleCrafting/forgetCursor
* @requiredAssets img/pSimpleCrafting/itemsListCursor
* @requiredAssets img/pSimpleCrafting/helpWindowBack
* @requiredAssets img/pSimpleCrafting/background
* @requiredAssets img/pSimpleCrafting/blocked
* @requiredAssets img/pSimpleCrafting/backgroundRecipe
* @requiredAssets img/pSimpleCrafting/backgroundAllRecipes
* @requiredAssets img/pSimpleCrafting/resultSuccess
* @requiredAssets img/pSimpleCrafting/resultFail
* @requiredAssets img/pSimpleCrafting/chanceTitle
 ...
 * 
 * @param recipeViewer2:j
 * @parent PKD_SimpleCrafting
 * @type note
 * @text Recipes List Scene
 * @desc Settings, in [param name]:[value] format. You can change values after :
 * @default "\"appearWithAnimation\": true\n\"closeButtonPosition\": { \"x\": 700, \"y\": 10 }\n\"forgeWindowPosition\": { \"x\": 448, \"y\": 74 }\n\"helpWindowPosition\": { \"x\": 133, \"y\": 495 }\n\"recipeListWindowRect\": {\"x\": 78, \"y\": 20, \"w\": 276, \"h\": 460}"
 * 
 * @param forgeSettings
 ...
 * 
 * @param isRemoveMissedMats:b
 * @parent forgeSettings
 * @type boolean
 * @text Is Auto Remove Materials?
 * @desc Remove materials from slots that are no longer in the inventory
 * @default false
 * 
 * @param chancesSystemGroup
 * @text Craft Chance System
 * 
 * @param isUseChanceSystem:b
 * @parent chancesSystemGroup
 * @type boolean
 * @text Is Use Chance System?
 * @desc Items can have chance to be crafted or not
 * @default false
 * 
 * @param chanceImgSuccess:j
 * @parent chancesSystemGroup
 * @text Success image
 * @type note
 * @desc Settings, in [param name]:[value] format. You can change values after :
 ...
 * 
 * @param chanceImgFail:j
 * @parent chancesSystemGroup
 * @text Fail image
 * @type note
 * @desc Settings, in [param name]:[value] format. You can change values after :
 * @default "\"appearWithAnimation\": true\n\"position\": { \"x\": 130, \"y\": 468 }\n\"appearSpeed\": 40\n\"stayTime\": 60"
 * 
 * @param chanceFailSE
 * @text Fail Sound
 * @parent chancesSystemGroup
 ...
 * @dir audio/se
 * @default Miss
 * @desc Sound effect when the item was not crafter (fail by chance)
 * 
 * @param chanceImgTitle:j
 * @parent chancesSystemGroup
 * @text Title image
 * @type note
 * @desc Settings, in [param name]:[value] format. You can change values after :
 * @default "\"position\": { \"x\": 65, \"y\": 284 }"
 * 
 * @param chanceTitleText:j
 * @parent chancesSystemGroup
 * @text Chance text
 * @type note
 * @desc Settings, in [param name]:[value] format. You can change values after :
 * @default "\"position\": { \"x\": 87, \"y\": -3 }\n\"size\": {\"w\": 60, \"h\": 20}\n\"textColor\": \"#ccc\"\n\"alignment\": \"left\"\n\"font\": { \"face\": \"Arial\", \"size\": 20, \"italic\": false }\n\"textFormat\": \"$%\""
 * 
 ...
 */
/*:ru
 * @plugindesc (v.1.2.1)[PRO] Система крафта
 * @author Pheonix KageDesu
 * @target MZ MV
 * @url https://kdworkshop.net/plugins/simple-crafting
 *
 ...
 *
 * - ShowAllCraftRecipes(); - открыть список всех рецептов в игре
 *      (рецепт - это предмет с <craftAutoRecipe:X> заметкой)
 *      Больше информации об этой заметке будет ниже
 *
 * - ShowMyCraftRecipes(); - открыть список всех рецептов игрока
 *      (предметы рецепты которые есть в инвентаре игрока) 
 *
 * ---------------------------------------------------------------------------
 ...
 *          номера слота.
 *  Можно использовать несколько <craft:ID,ID,ID,...> для одного предмета
 *
 ...
 *
  * - <craftFreePositions> - позиции материалов больше не имеют значение (только кол-во)
  *
  * - <craftChance:X> - шанс (от 1 до 100%) на успехо. В случае неудачи, материалы теряются всё равно
 *      (Нужно установить параметр плагина "Is Use Chance System?" на ВКЛ)
 *  Можно писать скрипты (формулы), а не только цифры
 *
 * Примеры: <craftChance:25> - 25%
            <craftChance:$gameVariables.value(33)> - Значение % из переменной 33
 *
 * ---------------------------------------------------------------------------
 ...
 * @type boolean
 * @text Is Auto Remove Materials?
 * @desc Удалять материалы из слотов, которых больше нет в инвентаре
 * @default false
 * 
 * @param chancesSystemGroup
 * @text Craft Chance System
 * @desc Система шансов создания предметов
 * 
 * @param isUseChanceSystem:b
 * @parent chancesSystemGroup
 * @type boolean
 * @text Is Use Chance System?
 * @desc Использовать систему?
 * @default false
 * 
 * @param chanceImgSuccess:j
 * @parent chancesSystemGroup
 * @text Success image
 * @type note
 * @desc Настройки для изображения успеха, в формате [имя параметра]:[значение]. Можно изменять значения после :
 * @default "\"appearWithAnimation\": true\n\"position\": { \"x\": 108, \"y\": 468 }\n\"appearSpeed\": 40\n\"stayTime\": 60"
 * 
 * @param chanceImgFail:j
 * @parent chancesSystemGroup
 * @text Fail image
 * @type note
 * @desc Настройки для изображения неудачи, в формате [имя параметра]:[значение]. Можно изменять значения после :
 * @default "\"appearWithAnimation\": true\n\"position\": { \"x\": 130, \"y\": 468 }\n\"appearSpeed\": 40\n\"stayTime\": 60"
 * 
 * @param chanceFailSE
 * @text Fail Sound
 ...
 * @dir audio/se
 * @default Miss
 * @desc Звуковой эффект, проигрываемый, когда предмет не был создан (неудача по шансу)
 * 
 * @param chanceImgTitle:j
 * @parent chancesSystemGroup
 * @text Title image
 * @type note
 * @desc Настройки для изображения заголовка, в формате [имя параметра]:[значение]. Можно изменять значения после :
 * @default "\"position\": { \"x\": 65, \"y\": 284 }"
 * 
 * @param chanceTitleText:j
 * @parent chancesSystemGroup
 * @text Chance text
 * @type note
 * @desc Настройки для текста о шансе, в формате [имя параметра]:[значение]. Можно изменять значения после :
 * @default "\"position\": { \"x\": 87, \"y\": -3 }\n\"size\": {\"w\": 60, \"h\": 20}\n\"textColor\": \"#ccc\"\n\"alignment\": \"left\"\n\"font\": { \"face\": \"Arial\", \"size\": 20, \"italic\": false }\n\"textFormat\": \"$%\""
 * 
 * @param spacer|endHolder @text‏‏‎ ‎@desc ===============================================
 * 
...
PKD_SimpleCrafting.Version = 120;
...
//Plugin KDCore builded by PKD PluginBuilder 2.2 - 08.12.2023
...
window.ShowAllCraftRecipes = function() {
  var e;
  try {
    SceneManager.push(Scene_PKD_SimpleCraftingRecipesList);
    return SceneManager.prepareNextScene(1); // * all
  } catch (error) {
    e = error;
    return KDCore.warning(e);
  }
...
window.ShowMyCraftRecipes = function() {
  var e;
  try {
    SceneManager.push(Scene_PKD_SimpleCraftingRecipesList);
    return SceneManager.prepareNextScene(0); // * only my
  } catch (error) {
    e = error;
    return KDCore.warning(e);
  }
...
(function() {
  ...
  _.recipeViewer2Settings = function() {
    return this._loader.getParam('recipeViewer2', {
      appearWithAnimation: true,
      closeButtonPosition: {
        x: 700,
        y: 10
  ...
  // * ------------------------------------------------------------------------------------------
  _.isRemoveMissedMaterials = function() {
    return this._loader.getParam('isRemoveMissedMats', false);
  };
  _.isUseChanceSystem = function() {
    return this._loader.getParam('isUseChanceSystem', false);
  };
  _.chanceFailSE = function() {
    return this._loader.getParam('chanceFailSE', "");
  };
  _.chanceSuccessImage = function() {
    return this._loader.getParam('chanceImgSuccess', {
      appearWithAnimation: true,
      position: {
        x: 108,
        y: 468
  ...
  _.chanceFailImage = function() {
    return this._loader.getParam('chanceImgFail', {
      appearWithAnimation: true,
      position: {
        x: 130,
        y: 468
  ...
  _.chanceTitleImage = function() {
    return this._loader.getParam('chanceImgTitle', {
      position: {
        x: 65,
        y: 284
      }
  ...
  _.chanceTitleText = function() {
    return this._loader.getParam('chanceTitleText', {
      position: {
        x: 87,
        y: -3
      },
...
(function() {
  ...
  // * Multiple
  _.isValidRecipeSet = function(recipes, forgeData, recipeConfig) {
    var e, i, len, r;
    try {
      for (i = 0, len = recipes.length; i < len; i++) {
        r = recipes[i];
        if (this.isValidRecipe(r, forgeData, recipeConfig)) {
          return true;
        }
      }
    } catch (error) {
  ...
  _.isValidRecipe = function(recipe, forgeData, config) {
    var e;
    try {
      while (recipe.length < 9) {
        recipe.push('0');
      }
      if ((config != null) && config.craftFreePositions === true) {
        return this.isValidRecipeFreePositions(recipe, forgeData);
      } else {
        // * Соблюдаем позиции
        return this.isValidRecipeStrict(recipe, forgeData);
      }
    } catch (error) {
      e = error;
      return KDCore.warning(e);
  ...
  _.isValidRecipeFreePositions = function(recipe, forgeData) {
    var countArrayElements, e, hasCount, key, requiredCount, value;
    try {
      countArrayElements = function(arr) {
        var counts, element, i, len, ref;
        counts = {};
        for (i = 0, len = arr.length; i < len; i++) {
          ...
          counts[element] = ((ref = counts[element]) != null ? ref : 0) + 1;
        }
        return counts;
      };
      requiredCount = countArrayElements(recipe);
      hasCount = countArrayElements(forgeData);
      for (key in requiredCount) {
        value = requiredCount[key];
        if ((hasCount[key] != null) && hasCount[key] === value) {
          continue;
        } else {
          return false;
        }
  ...
  _.isValidRecipeStrict = function(recipe, forgeData) {
    var a, b, checkLeft, e, i, id, index, len;
    try {
      checkLeft = 9;
      for (index = i = 0, len = recipe.length; i < len; index = ++i) {
        ...
        if (forgeData[index].toString() === id) {
          ...
          if (isFinite(forgeData[index]) && isFinite(id)) {
            a = parseInt(forgeData[index]);
            b = parseInt(id);
            if (a === b) {
              checkLeft--;
            }
  ...
  _.getCraftChance = function(item) {
    var e, value, valueRaw;
    try {
      if (item == null) {
        return 0;
      }
      if (item.meta.craftChance != null) {
        valueRaw = item.meta.craftChance;
...
(function() {
  ...
  _.pkdExtractCraftReceips = function() {
    ...
    try {
      ...
      for (j = 0, len = all.length; j < len; j++) {
        item = all[j];
        item.pCraftRecipes = DataManager.pGetAllCraftRecipesFromItem(item);
        item.pCraftConfig = {};
        if (item.meta.craftFreePositions != null) {
          results.push(item.pCraftConfig.craftFreePositions = true);
        } else {
          results.push(void 0);
        }
      }
  ...
  _.pGetAllItemsWithRecips = function() {
    ...
    try {
      all = $dataWeapons.concat($dataArmors).concat($dataItems);
      all = all.filter(function(i) {
        return (i != null) && (i.pCraftRecipes != null);
      });
    } catch (error) {
      e = error;
      KDCore.warning(e);
  ...
  _.pGetAllCraftRecipesFromItem = function(item) {
    var e, items, j, len, recipes, v, values;
    try {
      if (KDCore.Utils.hasMeta('craft', item)) {
        recipes = [];
        values = KDCore.Utils.getArrayOfValuesOfSameMeta('craft', item);
        if (values.length > 0) {
          for (j = 0, len = values.length; j < len; j++) {
            v = values[j];
            if (v.contains("craft")) { // * Igone other tags
              continue;
            }
            try {
              items = v.split(','); //.map (v) -> Number(v.trim())
              if (items.length > 0) {
                // * We can't check isFinite, because receipe item can be a1 or w1 (armor or weapon)
                //items = items.filter (i) -> !isNaN(parseFloat(i)) && isFinite(i)
                recipes.push(items);
              }
            } catch (error) {
              e = error;
  ...
  _.pGetCraftRecipeFromItem = function(item) {
    var e;
    try {
      if ((item.pCraftRecipes != null) && item.pCraftRecipes.length > 0) {
        return item.pCraftRecipes[0];
      }
      if (KDCore.Utils.hasMeta('craft', item)) {
...
Scene_PKD_SimpleCrafting = class Scene_PKD_SimpleCrafting extends Scene_MenuBase {
  ...
  isUseChanceSystem() {
    return PKDSCS.PP.isUseChanceSystem();
  }
  ...
  create() {
    ...
    if (this.isUseChanceSystem()) { //3
      this._createChanceInfo();
    }
...
(function() {
  ...
  _.checkRecipe = function() {
    var c, candidates, e, item, j, len;
    try {
      item = null;
      candidates = DataManager.pGetAllItemsWithRecips();
      for (j = 0, len = candidates.length; j < len; j++) {
        c = candidates[j];
        if (PKDSCS.Utils.isValidRecipeSet(c.pCraftRecipes, $gameTemp.pkdForgeItems, c.pCraftConfig)) {
          item = c;
          break;
        }
      }
  ...
  _.setRecipeItem = function(recipeItem) {
    ...
    try {
      ...
      this.showHelpForItem($gameTemp.pkdRecipeItem);
      try {
        if (this.isUseChanceSystem()) {
          if (recipeItem != null) {
            $gameTemp.pkdRecipeItemChance = PKDSCS.Utils.getCraftChance(recipeItem);
            return this._showChanceValue($gameTemp.pkdRecipeItemChance);
          } else {
            $gameTemp.pkdRecipeItemChance = null;
            return this._hideChanceValue();
          }
        }
      } catch (error) {
        e = error;
  ...
  _.craftCurrentRecipe = function() {
    ...
    try {
      ...
      if (this.isUseChanceSystem()) {
        try {
          if (KDCore.Utils.isChanceIsGood($gameTemp.pkdRecipeItemChance)) {
            $gameParty.gainItem($gameTemp.pkdRecipeItem, 1);
            this._playCraftSoundEffect(true);
            this._showResultInfo(true);
          } else {
            this._playCraftSoundEffect(false);
            this._showResultInfo(false);
          }
        } catch (error) {
          e = error;
          KDCore.warning(e);
      ...
      this._craftList.refresh();
      if (!this._craftForge.isCanForgeMore()) {
        this._removeMissingItemsFromForge();
      }
      this.checkRecipe();
    } catch (error) {
      e = error;
  ...
  _._removeMissingItemsFromForge = function() {
    var e, i, j, len, missedItemIndexes, results;
    try {
      if (!PKDSCS.PP.isRemoveMissedMaterials()) {
        return;
      }
      missedItemIndexes = this._craftForge.getMissedItemsIndexes();
      if (missedItemIndexes.length > 0) {
        results = [];
        for (j = 0, len = missedItemIndexes.length; j < len; j++) {
          i = missedItemIndexes[j];
          results.push($gameTemp.pkdForgeItems[i] = 0);
        }
        return results;
      }
    } catch (error) {
  ...
  _._playCraftSoundEffect = function(craftResultIsGood = true) {
    var e, seSoundName;
    try {
      if (craftResultIsGood === true) {
        seSoundName = DataManager.pGetCustomCraftSound($gameTemp.pkdRecipeItem);
        if (String.any(seSoundName)) {
          return KDCore.Utils.playSE(seSoundName);
        } else {
          try {
            return KDCore.Utils.playSE(this.settings().defaultCraftSoundSE);
          } catch (error) {
            e = error;
            return KDCore.warning(e);
          }
        ...
        return KDCore.Utils.playSE(PKDSCS.PP.chanceFailSE());
      }
    } catch (error) {
      e = error;
      return KDCore.warning(e);
...
(function() {
  ...
  //@[DEFINES]
  _ = Scene_PKD_SimpleCrafting.prototype;
  _._createChanceInfo = function() {
    var e;
    try {
      this._createChanceTitleSpr();
      this._createChanceValueTextSpr();
      return this._hideChanceValue();
    } catch (error) {
      e = error;
      return KDCore.warning(e);
  ...
  _._createChanceResultFailSpr = function() {
    var chanceResultFailSpr, e, s;
    try {
      s = PKDSCS.PP.chanceFailImage();
      chanceResultFailSpr = new Sprite_PKD_SC_ResultPopUp('resultFail', s);
      return chanceResultFailSpr;
    } catch (error) {
      e = error;
      return KDCore.warning(e);
    }
  };
  _._createChanceResultSuccessSpr = function() {
    var chanceResultSuccessSpr, e, s;
    try {
      s = PKDSCS.PP.chanceSuccessImage();
      chanceResultSuccessSpr = new Sprite_PKD_SC_ResultPopUp('resultSuccess', s);
      return chanceResultSuccessSpr;
    } catch (error) {
      e = error;
      return KDCore.warning(e);
    }
  };
  _._createChanceTitleSpr = function() {
    var e, s;
    try {
      s = PKDSCS.PP.chanceTitleImage();
      this._chanceTitleSpr = new KDCore.Sprite(ImageManager.loadPicturePSCS('chanceTitle'));
      this._chanceTitleSpr.move(s.position);
      return this._craftForge.addChild(this._chanceTitleSpr);
    } catch (error) {
      e = error;
      return KDCore.warning(e);
    }
  };
  _._createChanceValueTextSpr = function() {
    var e, p, s;
    try {
      s = PKDSCS.PP.chanceTitleText();
      p = {
        visible: true,
        size: s.size,
      ...
      this._chanceValueText = new KDCore.UI.Sprite_UIText(p);
      this._chanceValueText.move(s.position);
      this._chanceTitleSpr.addChild(this._chanceValueText);
      return this._chanceValueText._format = s.textFormat;
    } catch (error) {
      e = error;
      return KDCore.warning(e);
    }
  };
  _._showResultInfo = function(isSuccess) {
    var e;
    try {
      if (this._resultInfoSpr != null) {
        this._resultInfoSpr.removeFromParent();
        this._resultInfoSpr = null;
      }
      if (isSuccess === true) {
        this._resultInfoSpr = this._createChanceResultSuccessSpr();
      } else {
        this._resultInfoSpr = this._createChanceResultFailSpr();
      }
      return this.content.addChild(this._resultInfoSpr);
    } catch (error) {
      e = error;
      return KDCore.warning(e);
    }
  };
  _._showChanceValue = function(value) {
    var e;
    try {
      this._chanceTitleSpr.visible = true;
      return this._chanceValueText.draw(this._chanceValueText._format.replace("$", value));
    } catch (error) {
      e = error;
      return KDCore.warning(e);
    }
  };
  _._hideChanceValue = function() {
    var e;
    try {
      this._chanceTitleSpr.visible = false;
      return this._chanceValueText.draw("");
    } catch (error) {
      e = error;
      return KDCore.warning(e);
    }
...
Scene_PKD_SimpleCraftingRecipeDisplay = class Scene_PKD_SimpleCraftingRecipeDisplay extends Scene_PKD_SimpleCrafting {
  ...
  start() {
    super.start();
    this.showRecipeAtStart();
  }

  showRecipeAtStart() {
    return this.showRecipeData();
  }

  showRecipeData() {
    $gameTemp.pkdRecipeItem = this.dbItem;
    this.setRecipeItem(this.dbItem);
    return this.showStaticRecipe();
  }
...
// Generated by CoffeeScript 2.6.1
var Scene_PKD_SimpleCraftingRecipesList;

Scene_PKD_SimpleCraftingRecipesList = class Scene_PKD_SimpleCraftingRecipesList extends Scene_PKD_SimpleCraftingRecipeDisplay {
  constructor() {
    super(...arguments);
    this._lastDisplayedRecipeIndex = -1;
    return;
  }
  ...
  update() {
    ...
    super.update();
    try {
      return this._updateShowRecipeByIndexFromList();
    } catch (error) {
      e = error;
      return KDCore.warning(e);
    }
  ...
  prepare(_recipesToShowMode) {
    this._recipesToShowMode = _recipesToShowMode;
  }

  settingsExt() {
    return PKD_SimpleCrafting.PP.recipeViewer2Settings();
  }
  ...
  _afterAnimationDone() {
    ...
    this._recipesWindow.select(0);
  }

  _backgroundImage() {
    return 'backgroundAllRecipes';
  }

  _createWindows() {
    super._createWindows();
    this._createRecipeListWindow();
    return this._modifyCraftForgeForOnlyDisplayInfo();
  }

  _createRecipeListWindow() {
    this._recipesWindow = new Window_PKD_CraftRecipesList(this._craftRecipesWindowRect());
    this._recipesWindow.setRecipesToShow(this._recipesToShowMode);
    this._recipesWindow.setHandler("ok", this.onRecipeOk.bind(this));
    this._recipesWindow.setHandler("cancel", this.onCloseButtonClick.bind(this));
    this._recipesWindow.activate();
    this._recipesWindow.refresh();
    this.content.addChild(this._recipesWindow);
  }
  ...
  onRecipeOk() {
    ...
    try {
      this._craftForge.refresh();
      this._craftForge.activate();
      return this._craftForge.safeSelect();
    } catch (error) {
      e = error;
      KDCore.warning(e);
      return this._recipesWindow.activate();
    }
  }
  ...
  _modifyCraftForgeForOnlyDisplayInfo() {
    var e;
    try {
      return this._craftForge.setHandler("cancel", this.onBackFromForgeToList.bind(this));
    } catch (error) {
      e = error;
      return KDCore.warning(e);
    }
  ...
  onBackFromForgeToList() {
    var e;
    try {
      this._craftForge.select(-1);
      this._recipesWindow.activate();
      return this._recipesWindow.refresh();
    } catch (error) {
      e = error;
      return KDCore.warning(e);
    }
  ...
  _craftRecipesWindowRect() {
    var e, h, w, x, y;
    try {
      ({x, y, w, h} = this.settingsExt().recipeListWindowRect);
      return new Rectangle(x, y, w, h);
    } catch (error) {
      e = error;
      KDCore.warning(e);
  ...
  _updateShowRecipeByIndexFromList() {
    var index, itemId, recipeItem;
    if (this._recipesWindow == null) {
      return;
    }
    index = this._recipesWindow.index();
    if (index !== this._lastDisplayedRecipeIndex) {
      this._lastDisplayedRecipeIndex = index;
      recipeItem = this._recipesWindow.itemAt(index);
      if (recipeItem == null) {
        return;
      }
      itemId = DataManager.pExtractAutoRecipeFromItem(recipeItem);
      if (itemId != null) {
        this.dbItem = PKD_SimpleCrafting.Utils.convertForgeItemIdToGameItem(itemId.toString());
        if (this.dbItem != null) {
          this.recipe = DataManager.pGetCraftRecipeFromItem(this.dbItem);
        } else {
          this.recipe = null;
        }
        if ((this.dbItem != null) && (this.recipe != null)) {
          this.showRecipeData();
        }
      }
    }
...
// Generated by CoffeeScript 2.6.1
var Sprite_PKD_SC_ResultPopUp;

Sprite_PKD_SC_ResultPopUp = class Sprite_PKD_SC_ResultPopUp extends KDCore.Sprite {
  constructor(imageName, settings) {
    super();
    this._create(imageName, settings);
    if (settings.stayTime > 0) {
      this._removeThread = new KDCore.TimedUpdate(settings.stayTime, this._startDispose.bind(this));
    }
  ...
  _startDispose() {
    ...
    try {
      this.disapper(75);
      return this._removeThread = new KDCore.TimedUpdate(120, this._endDispose.bind(this));
    } catch (error) {
      e = error;
      return KDCore.warning(e);
    }
  ...
  _create(imageName, settings) {
    var e;
    try {
      this.bitmap = ImageManager.loadPicturePSCS(imageName);
      if (settings.appearWithAnimation === true && settings.appearSpeed > 0) {
        return this.appear(settings.appearSpeed);
      }
    } catch (error) {
      e = error;
      return KDCore.warning(e);
  ...
  update() {
    var ref;
    super.update();
    return (ref = this._removeThread) != null ? ref.update() : void 0;
  }
...
Window_PKD_CraftForge = class Window_PKD_CraftForge extends Window_Selectable {
  ...
  isCanForgeMore() {
    return this.getMissedItemsIndexes().length === 0;
  }

  getMissedItemsIndexes() {
    var i, j, missed, ref;
    missed = [];
...
// Generated by CoffeeScript 2.6.1
//Window_PKD_CraftMaterialsList
var Window_PKD_CraftRecipesList;

Window_PKD_CraftRecipesList = class Window_PKD_CraftRecipesList extends Window_PKD_CraftMaterialsList {
  constructor() {
    super(...arguments);
  }

  setRecipesToShow(_recipesToShowMode) {
    this._recipesToShowMode = _recipesToShowMode;
  }

  //$[OVER]
  _pDrawRemoveItemCommand() {} // * EMPTY
  ...
  //$[OVER]
  makeItemList() {
    this._data = this.filterOnlyRecipes($dataItems);
  }

  filterOnlyRecipes(items) {
    return items.filter((item) => {
      return KDCore.Utils.hasMeta('craftAutoRecipe', item) && this.includes(item);
    });
  }
...
//Plugin PKD_SimpleCrafting builded by PKD PluginBuilder 2.2 - 15.12.2023