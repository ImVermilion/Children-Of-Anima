 ...
 * rename the js file.
 * -----------------------------Latest Version---------------------------------
 * Hi all, this latest version added some integrations with other [CGMZ]
 * plugins. You can now show a [CGMZ] Scene Backgrounds background image, which
 * allows for scrolling backgrounds or weather. You can also show the [CGMZ]
 * Controls Window which lets the player know gamepad or keyboard controls for
 * the scene, depending on their last input. The order with [CGMZ] Toast
 * Manager is no longer important in the Plugin Manager.
 *
 * This update also added options to configure sound effect volume, pitch, and
 * pan settings.
 *
 * The Fast Travel scene is now more customizable as well. You can show the
 * image window at the top of the scene now with the description window below
 * it. You can also choose how many lines of text the description window can
 * accommodate.
 *
 * Version 1.1.1
...
CGMZ.Versions["Fast Travel"] = "1.1.1";
CGMZ.FastTravel = {};
...
CGMZ_FastTravelPoint.prototype.initialize = function(point) {
	...
	this._travelSe = CGMZ_Utils.parseSoundEffectJSON(point["Travel Sound Effect"], '[CGMZ] Fast Travel');
	this._toastSe = CGMZ_Utils.parseSoundEffectJSON(point["Toast Sound Effect"], '[CGMZ] Fast Travel');
};
//-----------------------------------------------------------------------------
...
CGMZ_FastTravelPoint.prototype.setupDiscoverToast = function() {
	...
	toast.lineOne = CGMZ.FastTravel.ToastText;
	toast.lineTwo = this._name;
	if(this._toastSe?.name) toast.SE = this._toastSe;
...
CGMZ_Scene_FastTravel.prototype.onListOk = function() {
	...
	if(!fastTravel) {
		...
		if(fastTravel._travelSe?.name) AudioManager.playSe(fastTravel._travelSe);
...
CGMZ_Window_FastTravelImage.prototype.drawItemImage = function() {
	this._sprite.hide();
	const imageData = CGMZ_Utils.getImageData(this._item._image, "img");
