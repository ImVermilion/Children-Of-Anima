...
/*:
 * @plugindesc (v.1.3)[PRO] Extended Loot
 * @author Pheonix KageDesu
 * @target MZ MV
 * @url https://kdworkshop.net/plugins/extended-loot
 *
 * @help
 * ---------------------------------------------------------------------------
 * 
 ...
 *  (item should be Game Item object, color (HEX string) is optional
 *
 * Example: EL_ShowScreenPopUpForItem($dataItems[7], 1, '#ffffff');
 *
 * EL_ShowScreenPopUpCustom(iconIndex, text, count, color) - show custom PopUp
 * with specified text and iconIndex
 *
 * Exmaple: EL_ShowScreenPopUpCustom(44, 'Quest Updated', 0, '#ff88ff')
 *
 * ===========================================================================
 * Visual Drop
 * ===========================================================================
 ...
 *
 * If you want to disable auto pick up for some items, you can add notetag:
 * <noAutoLoot>
 *
 * You can add special sound for item drop via notetag:
 * <lootDropSound:NAME>, where NAME - name of sound from Se folder
 *
 ...
 * 
 * @param isShowOnScreenPopUps:b
 * @parent isTreasuresPopUpActive:b
 * @text Show On-Screen Pop-ups
 * @type boolean
 * @default true
 * @desc Enable or disable on-screen pop-ups for loot items.
 * 
 * @param isContinueShowPopUpsAfterMapChange:b
 * @text Continue Show Pop-ups After stop Map
 * @parent isShowOnScreenPopUps:b
 * @type boolean
 * @default false
 * @desc If enabled, on-screen pop-ups will continue to show even after open Menu.
 * 
 * @param isRiseCountForSameItem:b
 * @text Rise Count for Same Item
 * @parent isShowOnScreenPopUps:b
 * @type boolean
 * @default true
 * @desc If enabled, the count for the same item in on-screen pop-ups will increase.
 * 
 * @param screenPopUpQueueDelayInMs:i
 * @text Pop-up Queue Delay (in milliseconds)
 * @parent isShowOnScreenPopUps:b
 * @type number
 * @min 0
 * @default 500
 * @desc The delay between each on-screen pop-up in the queue, in milliseconds.
 * 
 ...
 * 
 * @param isNetworkGlobalVisualDrop:b
 * @parent isVisualDropActive:b
 * @text Share Visual Drop over network?
 * @type boolean
 * @desc In NETZ multiplayer all players will see visual drop and can take it
 * @default false
 * 
 ...
 * ===========================================================================
 * Всплывающие сообщения (экран)
 * ===========================================================================
 * EL_ClearAllScreenPopUps(); - очистить все всплывающие сообщения на экране
 * EL_ShowScreenPopUpForItem(item, count, [color]) - показать всплывающее сообщение для предмета
 *  (предмет должен быть объектом Game Item, цвет (HEX строка) является необязательным
 *
 * Пример: EL_ShowScreenPopUpForItem($dataItems[7], 1, '#ffffff');
 *
 * EL_ShowScreenPopUpCustom(iconIndex, text, count, color) - показать пользовательское всплывающее сообщение
 * с указанным текстом и индексом иконки
 *
 * Пример: EL_ShowScreenPopUpCustom(44, 'Миссия обновлена', 0, '#ff88ff')
 * ===========================================================================
 * Визуальный дроп (выпадение вещей)
 * ===========================================================================
 *   Параметры плагина -> Visual Drop (установите значение в ВКЛ для активации)
 *
 * Каждый раз, когда вы используете команду события "Изменить предмет (оружие, броню, золото)",
 * предметы выпадают (выпрыгивают) из события на землю вокруг
 *
 * - Некоторые предметы будут автоматически подбираться (вы можете изменить радиус автоматического подбора через
 * Параметры плагина или отключить его).
 * - Чтобы подобрать определенный предмет, игрок должен щелкнуть на него мышью (или коснуться).
 *
 * Если вы хотите отключить автоматическое подбирание для некоторых предметов, вы можете добавить тег:
 * <noAutoLoot>
 *
 * Вы можете добавить специальный звук для выпадения предмета с помощью notetag:
 * <lootDropSound:NAME>, где NAME - имя звука из папки SE
 *
 * Если вы хотите дать игроку некоторые предметы (оружие, броню) без визуального выпадения,
 * вы можете отключить систему визуального выпадения в игре, используя Вызов скрипта:
 * - VD_setShouldDrop(false); - отключить визуальное выпадение в игре
 * - VD_setShouldDrop(true); - активировать снова
 *
 * Если у вас есть PRO-версия плагина, вы можете настроить визуальное выпадение через
 * Параметры плагина
 * ===========================================================================
 * Random Loot
 * ===========================================================================
 ...
 * @type item
 * @default 0
 * @desc Предмет для отображения золота (иконка, название) в списке добычи и визуальном выпадении.
 * 
 * @param goldItemTextColor
 * @parent goldItemSettingsGroup
 * @text Цвет текста золотого предмета
 * @default #dea712
 * @desc Цвет текста названия золотого предмета в формате HEX для списка добычи и визуального выпадения.
 * 
 * @param spacer|MainSettings @text‏‏‎ ‎@desc ===============================================
 * 
 * @param isLootWindowActive:b
 ...
 * @type boolean
 * @default true
 * @desc Использовать окно добычи в игре?
 * 
 * @param isCloseLootWindowOnESC:b
 * @text Закрыть по клавише отмены?
 * @parent isLootWindowActive:b
 * @type boolean
 * @default true
 * @desc Закрывать окно добычи по клавише отмены (ESC)? (или правый клик мыши)
 * 
 * @param isAutoCloseAllowed:b
 ...
 * @type boolean
 * @default true
 * @desc Автоматически закрывать окно добычи, когда предметы закончатся?
 * 
 * @param autoCloseDelay:int
 * @parent isAutoCloseAllowed:b
 * @text Задержка автоматического закрытия
 * @type number 
 * @min 0
 * @max 120
 * @desc Задержка перед автоматическим закрытием списка добычи, когда предметы закончатся. 0 - мгновенно.
 * @default 10
 * 
 * @param isAddTakeAllButton:b
 * @text Опция "Взять все"?
 * @parent isLootWindowActive:b
 * @type boolean
 * @default true
 * @desc Добавить опцию "Взять все" в список?
 * 
 * @param takeAllCmd:struct
 * @parent isAddTakeAllButton:b
 * @text Команда "Взять все"
 * @type struct<str17>
 * @desc [PRO] Настройки команды "Взять все"
 * @default {"title:str":"[Взять все]","iconIndex:i":"75","color:str":"#269dde"}
 * 
 * @param takeAllDelay:int
 * @parent isAddTakeAllButton:b
 * @type number
 ...
 * @text Задержка
 * @default 5
 * @desc Задержка (в кадрах) между взятием каждого предмета при выборе опции "Взять все". 0 - без задержки.
 * 
 * @param takeAllSE
 * @parent isAddTakeAllButton:b
 * @text Звук "Взять все"
 ...
 * @dir audio/se
 * @require 1
 * @desc Звуковой эффект при нажатии на опцию "Взять все" (взять все предметы)
 * 
 * @param takeAllCommandKeyboardKey
 * @parent isAddTakeAllButton:b
 * @text Клавиша на клавиатуре
 * @desc Клавиша на клавиатуре для опции "Взять все" (в нижнем регистре)
 * @default f
 * 
 * @param takeAllCommandGamepadKey
 * @parent isAddTakeAllButton:b
 * @text Кнопка на геймпаде
 * @desc Кнопка на геймпаде для опции "Взять все"
 * @type select
 * @option A
 * @option X
 * @option Y
 ...
 * @param takeItemSE
 * @parent isLootWindowActive:b
 * @text Звук при взятии предмета
 * @type file
 * @dir audio/se
 * @require 1
 * @desc Звуковой эффект при взятии предмета из списка добычи
 * 
 * @param takeGoldSE
 * @parent isLootWindowActive:b
 * @text Звук при взятии золота
 * @type file
 * @dir audio/se
 * @require 1
 * @desc Звуковой эффект при взятии золота из списка добычи
 * 
 * @param lootWindowVisualGroup
 * @text Настройки окна добычи
 * @parent isLootWindowActive:b
 * 
 * @param lootWindowSettings:struct
 * @text Окно списка добычи
 * @type struct<str11>
 * @parent lootWindowVisualGroup
 * @desc [PRO] Настройки окна списка добычи
 * @default {"mainWindowSize:s":"{\"w:int\":\"220\",\"h:int\":\"360\"}","isAbsolutePosition:b":"true","position:s":"{\"x:str\":\"Graphics.width / 2  - 110\",\"y:str\":\"Graphics.height / 2 - 180\"}","isDraggable:b":"true","showCloseButton:b":"true","closeButtonPosition:s":"{\"x:int\":\"81\",\"y:int\":\"356\"}","headerText:s":"{\"visible:bool\":\"true\",\"size:struct\":\"{\\\"w:int\\\":\\\"220\\\",\\\"h:int\\\":\\\"80\\\"}\",\"font:struct\":\"{\\\"face:str\\\":\\\"\\\",\\\"size:int\\\":\\\"30\\\",\\\"italic:bool\\\":\\\"false\\\"}\",\"margins:struct\":\"{\\\"x:int\\\":\\\"0\\\",\\\"y:int\\\":\\\"-7\\\"}\"}","headerDefaultImage:s":"{\"name:str\":\"defaultHeader\",\"margins:struct\":\"{\\\"x:int\\\":\\\"-25\\\",\\\"y:int\\\":\\\"-44\\\"}\"}"}
 * 
 * @param listWindowSettings:struct
 * @parent lootWindowVisualGroup
 * @text Окно списка предметов
 * @type struct<str16>
 * @desc [PRO] Настройки окна списка предметов
 * @default {"listWindowSize:s":"{\"w:int\":\"220\",\"h:int\":\"318\"}","listWindowMargins:s":"{\"x:int\":\"0\",\"y:int\":\"32\"}","listItem:s":"{\"color:str\":\"#ffffff\",\"font:s\":\"{\\\"face:str\\\":\\\"\\\",\\\"size:int\\\":\\\"22\\\",\\\"italic:bool\\\":\\\"false\\\"}\"}","itemCount:s":"{\"color:str\":\"#dbd9b2\",\"font:s\":\"{\\\"face:str\\\":\\\"\\\",\\\"size:int\\\":\\\"20\\\",\\\"italic:bool\\\":\\\"false\\\"}\"}","goldCount:s":"{\"color:str\":\"#dea712\",\"font:s\":\"{\\\"face:str\\\":\\\"\\\",\\\"size:int\\\":\\\"22\\\",\\\"italic:bool\\\":\\\"false\\\"}\"}","itemCountText:str":"x%0","goldCountText:str":"%0"}
 * 
 * @param spacer|treasurePopUpSettings @text‏‏‎ ‎@desc ===============================================
 * 
 * @param isTreasuresPopUpActive:b
 * @text Всплывающие окна
 * @type boolean
 * @default true
 * @desc Использовать всплывающие окна в игре?
 * 
 * @param popUpMainSettings:s
 * @parent isTreasuresPopUpActive:b
 * @text Конфигурация всплывающих окон
 * @type struct<str8>
 * @desc [PRO] Конфигурация всплывающих окон
 * @default {"margins:s":"{\"x:int\":\"0\",\"y:int\":\"-70\"}","opacityStep:i":"10","moveStep:i":"1","stayTime:i":"80","dyBetweenLines:i":"20","appearSE:str":""}
 * 
 * @param popUpLineSettings:s
 * @parent isTreasuresPopUpActive:b
 * @text Всплывающий предмет
 * @type struct<str7>
 * @desc [PRO] Визуальные настройки и конфигурация всплывающего предмета
 * @default {"text:s":"{\"visible:bool\":\"true\",\"size:struct\":\"{\\\"w:int\\\":\\\"80\\\",\\\"h:int\\\":\\\"20\\\"}\",\"alignment:str\":\"left\",\"font:struct\":\"{\\\"face:str\\\":\\\"\\\",\\\"size:int\\\":\\\"12\\\",\\\"italic:bool\\\":\\\"false\\\"}\",\"margins:struct\":\"{\\\"x:int\\\":\\\"0\\\",\\\"y:int\\\":\\\"0\\\"}\",\"outline:struct\":\"{\\\"color\\\":\\\"#000000\\\",\\\"width:int\\\":\\\"2\\\"}\",\"textColor:str\":\"#EAEAEA\"}","back:s":"{\"visible:bool\":\"true\",\"size:struct\":\"{\\\"w:int\\\":\\\"0\\\",\\\"h:int\\\":\\\"20\\\"}\",\"fillColor:str\":\"#000000\",\"fillOpacity:i\":\"156\",\"borderColor:str\":\"#000000\",\"borderThickness:i\":\"0\",\"borderOpacity:i\":\"255\"}","backMargins:s":"{\"x:int\":\"0\",\"y:int\":\"0\"}","icon:s":"{\"visible:bool\":\"true\",\"index:i\":\"0\",\"size:i\":\"14\"}","countTextStr:str":"x%0","countText:s":"{\"visible:bool\":\"true\",\"size:struct\":\"{\\\"w:int\\\":\\\"50\\\",\\\"h:int\\\":\\\"20\\\"}\",\"alignment:str\":\"left\",\"font:struct\":\"{\\\"face:str\\\":\\\"\\\",\\\"size:int\\\":\\\"11\\\",\\\"italic:bool\\\":\\\"false\\\"}\",\"margins:struct\":\"{\\\"x:int\\\":\\\"0\\\",\\\"y:int\\\":\\\"2\\\"}\",\"outline:struct\":\"{\\\"color\\\":\\\"#000000\\\",\\\"width:int\\\":\\\"2\\\"}\",\"textColor:str\":\"#dbd9b2\"}"}
 * 
 * @param maxPopItemTextLength:int
 ...
 * @min 10
 * @default 15
 * @desc Можно ограничить длину названия предмета, чтобы не создавать слишком длинные всплывающие окна
 * 
 * @param isShowOnScreenPopUps:b
 * @parent isTreasuresPopUpActive:b
 * @text Показывать всплывающие окна на экране
 * @type boolean
 * @default true
 * @desc Включить или отключить всплывающие окна на экране для предметов добычи.
 * 
 * @param isContinueShowPopUpsAfterMapChange:b
 * @text Продолжать показывать всплывающие окна после смены карты
 * @parent isShowOnScreenPopUps:b
 * @type boolean
 * @default false
 * @desc Если включено, всплывающие окна будут продолжать отображаться даже после открытия меню.
 * 
 * @param isRiseCountForSameItem:b
 * @text Увеличивать количество для одного предмета
 * @parent isShowOnScreenPopUps:b
 * @type boolean
 * @default true
 * @desc Если включено, количество одного предмета во всплывающих окнах будет увеличиваться.
 * 
 * @param screenPopUpQueueDelayInMs:i
 * @text Задержка очереди всплывающих окон (в миллисекундах)
 * @parent isShowOnScreenPopUps:b
 * @type number
 * @min 0
 * @default 500
 * @desc Задержка между каждым всплывающим окном на экране в очереди, в миллисекундах.
 * 
 * @param spacer|VisualDropSettings @text‏‏‎ ‎@desc ===============================================
 * 
 * @param isVisualDropActive:b
 * @text Визуальное падение
 * @type boolean
 * @default true
 * @desc Использовать визуальную систему падения предметов в игре?
 * 
 * @param visualDropMainSettings:s
 * @parent isVisualDropActive:b
 * @text Конфигурация визуального падения
 * @type struct<str14>
 * @desc [PRO] Конфигурация визуальной системы падения предметов
 * @default {"autoLootDistanceInPx:i":"80","clickLootDistanceInPx:i":"160","moveSpeed:i":"5","flowAnimation:s":"{\"speed:i\":\"0.1\",\"height:i\":\"1.2\"}","gainItemSE:str":"","dropItemSE:str":"","jumpX:s":"{\"min:i\":\"0.1\",\"max:i\":\"1.5\"}","jumpY:s":"{\"min:i\":\"0.1\",\"max:i\":\"1.5\"}","dropDelay:s":"{\"min:i\":\"0.00\",\"max:i\":\"500.00\"}","hintTextOnlyWhenHovered:b":"true","hintTextMargins:s":"{\"x:int\":\"12\",\"y:int\":\"-20\"}"}
 * 
 * @param visualDropItemSettings:s
 * @parent isVisualDropActive:b
 * @text Визуальное падение предмета
 * @type struct<str7>
 * @desc [PRO] Визуальные настройки и конфигурация падения предмета
 * @default {"text:s":"{\"visible:bool\":\"true\",\"size:struct\":\"{\\\"w:int\\\":\\\"80\\\",\\\"h:int\\\":\\\"20\\\"}\",\"alignment:str\":\"left\",\"font:struct\":\"{\\\"face:str\\\":\\\"\\\",\\\"size:int\\\":\\\"12\\\",\\\"italic:bool\\\":\\\"false\\\"}\",\"margins:struct\":\"{\\\"x:int\\\":\\\"0\\\",\\\"y:int\\\":\\\"0\\\"}\",\"outline:struct\":\"{\\\"color\\\":\\\"#000000\\\",\\\"width:int\\\":\\\"2\\\"}\",\"textColor:str\":\"#EAEAEA\"}","back:s":"{\"visible:bool\":\"true\",\"size:struct\":\"{\\\"w:int\\\":\\\"0\\\",\\\"h:int\\\":\\\"20\\\"}\",\"fillColor:str\":\"#000000\",\"fillOpacity:i\":\"220\",\"borderColor:str\":\"#000000\",\"borderThickness:i\":\"0\",\"borderOpacity:i\":\"255\"}","backMargins:s":"{\"x:int\":\"0\",\"y:int\":\"0\"}","icon:s":"{\"visible:bool\":\"true\",\"index:i\":\"0\",\"size:i\":\"24\"}","countTextStr:str":"x%0","countText:s":"{\"visible:bool\":\"true\",\"size:struct\":\"{\\\"w:int\\\":\\\"50\\\",\\\"h:int\\\":\\\"20\\\"}\",\"alignment:str\":\"left\",\"font:struct\":\"{\\\"face:str\\\":\\\"\\\",\\\"size:int\\\":\\\"11\\\",\\\"italic:bool\\\":\\\"false\\\"}\",\"margins:struct\":\"{\\\"x:int\\\":\\\"0\\\",\\\"y:int\\\":\\\"2\\\"}\",\"outline:struct\":\"{\\\"color\\\":\\\"#000000\\\",\\\"width:int\\\":\\\"2\\\"}\",\"textColor:str\":\"#dbd9b2\"}"}
 * 
 * @param isNetworkGlobalVisualDrop:b
 * @parent isVisualDropActive:b
 * @text Общее визуальное падение по сети?
 * @type boolean
 * @desc Если включено, все игроки в многопользовательской игре будут видеть и могут подобрать предметы
 * @default false
 * 
 * @param spacer|RaritySettings @text‏‏‎ ‎@desc ===============================================
 * 
 * @param raritySystem:b
 * @text Система редкости
 * @type boolean
 * @default true
 * @desc Использовать систему редкости в игре?
 * 
 * @param itemsRarityGroup:structA
 * @parent raritySystem:b
 * @text Уровни редкости предметов
 * @type struct<Rarity>[]
 * @default []
 * @desc Уровни редкости
 * 
 * @param isRarityInDefault:b
 * @parent raritySystem:b
 * @text Использовать в окнах?
 * @type boolean
 * @default true
 * @desc Отображать редкость предмета (цвет названия и дополнительные иконки) в стандартных окнах RPG Maker (сценах)?
 * 
 * @param spacer|endHolder @text‏‏‎ ‎@desc ===============================================
 * 
 * 
...
KDCore._fileVersion = '3.5.5';

KDCore.nuiVersion = '1.1.1';
...
KDCore.registerLibraryToLoad(function() {
  ...
  return KDCore.AnimationKeyLine = class AnimationKeyLine {
    ...
    _parseKeyFrames(keyframes, func) {
      var dpValue, duration, e, endValue, endValues, index, key, keyframesOutput, keys, kf, negative, prevKey, r, result, resultValue, startValue, value;
      try {
        keyframesOutput = [];
        endValues = [];
        keys = [];
...
//Plugin KDCore builded by PKD PluginBuilder 2.2.1 - 22.06.2024

PKD_ExtendedLoot.MZ3D_Patch = function () {
...
(function(){
    ...
    DataManager.loadDatabase = function () {
        ...
        PKD_ExtendedLoot.LoadNetworkCode();
...
(function() {
  var _;
  //@[DEFINES]
  _ = PKD_ExtendedLoot.PP;
  // * Loot Window активен? (вообще)
  _.isLootWindowActive = function() {
    return this._loader.getParam("isLootWindowActive", true);
  };
  // * Treasures Pop Up активен? (вообще)
  _.isTreasuresPopUpActive = function() {
    return this._loader.getParam("isTreasuresPopUpActive", true) && !$gameSystem._pelNoPopUp;
  };
  // * Visual Drop активен? (вообще)
  _.isVisualDropActive = function() {
    return this._loader.getParam("isVisualDropActive", true);
  };
  // * Rarity активен? (вообще)
  _.isRaritySystemActive = function() {
    return this._loader.getParam("raritySystem", true);
  };
  // * Закрывать окно при нажатии на ESC (правая кнопка мыши)
  _.isCloseLootWindowOnESC = function() {
    return this._loader.getParam("isCloseLootWindowOnESC", true);
  };
  //?VERSION
  _.getLootWindowSettings = function() {
    return {
      mainWindowSize: {
        w: 220,
        h: 360
      ...
      position: { // * Если не Absolute, то используется как Margin
        x: "Graphics.width / 2  - 110",
        y: "Graphics.height / 2 - 180"
      },
      isDraggable: true,
  ...
  //?VERSION
  _.getItemListWindowSettings = function() {
    return {
      listWindowSize: {
        w: 220,
        h: 318
  ...
  _.isAddTakeAllButton = function() {
    return this._loader.getParam("isAddTakeAllButton", true);
  };
  _.isAutoCloseAllowed = function() {
    return this._loader.getParam("isAutoCloseAllowed", true);
  };
  // * Задержка автоматического закрытия, когда был взят последний предмет
  _.closeDelay = function() {
    return this._loader.getParam("autoCloseDelay", 10);
  };
  // * Звук, когда нажимаешь взять предмет
  _.takeItemSE = function() {
    return this._loader.getParam("takeItemSE", "");
  };
  // * Звук, когда нажимаешь взять золото
  _.takeGoldSE = function() {
    return this._loader.getParam("takeGoldSE", "");
  };
  // * Звук, когда нажимаешь "взять всё"
  _.takeAllSE = function() {
    return this._loader.getParam("takeAllSE", "");
  };
  // * Задержка перед каждым "подбором" предмета в режиме TakeAll
  _.takeAllDelay = function() {
    return this._loader.getParam("takeAllDelay", 5);
  };
  //?VERSION
  _.getTakeAllCommandSettings = function() {
    return {
      title: "[Take All]",
      iconIndex: 75,
      color: "#269dde"
  ...
  _.takeAllCommandKeyboardKey = function() {
    return this._loader.getParam("takeAllCommandKeyboardKey", "f");
  };
  _.takeAllCommandGamepadKey = function() {
    return this._loader.getParam("takeAllCommandGamepadKey", "X");
  };
  _.goldItemId = function() {
    return this._loader.getParam("goldItemId", 0);
  };
  _.goldItemTextColor = function() {
    return this._loader.getParam("goldItemTextColor", "#dea712");
  };
  //?VERSION
  _.getItemsRaritySettings = function() {
    var levels;
    levels = this._loader.getParam("itemsRarityGroup", [
      {
        color: "#a1e065",
        icon: 0
      },
  ...
  _.isDrawRarityInDefaultWindows = function() {
    return this._loader.getParam("isRarityInDefault", true);
  };
  //?VERSION
  _.getPopUpSettings = function() {
    return {
      margins: {
        x: 0,
        y: -70
  ...
  //?VERSION
  _.getPopUpLineSettings = function() {
    return {
      text: {
        visible: true,
        size: {
  ...
  //?VERSION
  _.getVisualDropItemSettings = function() {
    return {
      text: {
        visible: true,
        size: {
  ...
  //?VERSION
  _.getVisualDropSettings = function() {
    return {
      autoLootDistanceInPx: 80,
      clickLootDistanceInPx: 160,
      moveSpeed: 5,
      flowAnimation: {
        speed: 0.1,
        height: 1.2
  ...
  _.getMaxPopUpItemTextLength = function() {
    return this._loader.getParam("maxPopItemTextLength", 15);
  };
  _.isNetworkGlobalVisualDrop = function() {
    return this._loader.getParam("isNetworkGlobalVisualDrop", false) && Imported.Alpha_NETZ === true;
  };
  // * Показывать ли PopUp на экране
  _.isShowOnScreenPopUps = function() {
    return this._loader.getParam("isShowOnScreenPopUps", true);
  };
  _.isContinueShowPopUpsAfterMapChange = function() {
    return this._loader.getParam("isContinueShowPopUpsAfterMapChange", true);
  };
  _.screenPopUpQueueDelayInMs = function() {
    return this._loader.getParam("screenPopUpQueueDelayInMs", 500);
  };
  _.isRiseCountForSameItem = function() {
    return this._loader.getParam("isRiseCountForSameItem", true);
  };
})();
...
(function() {
  ...
  _.LoadCompatibilityPatches = function() {
    ...
    (function() {      // ■ END PKD_MI.LIBS.MapUserChestController.coffee
      //---------------------------------------------------------------------------
      var __alias_CMGIC;
    ...
    return PKD_ExtendedLoot.MZ3D_Patch();
  };
})();
...
//╒═════════════════════════════════════════════════════════════════════════╛
//---------------------------------------------------------------------------
PKD_ExtendedLoot.isPro = function() {
  return true;
};
...
(function() {
  ...
  _ = PKD_ExtendedLoot.PP;
  _.getLootWindowSettings = function() {
    return this._loader.getParam("lootWindowSettings", {
      mainWindowSize: {
        w: 220,
        h: 360
      },
  ...
  _.getTakeAllCommandSettings = function() {
    return this._loader.getParam("takeAllCmd", {
      title: "[Take All]",
      iconIndex: 75,
      color: "#269dde"
    });
  ...
  _.getPopUpSettings = function() {
    return this._loader.getParam("popUpMainSettings", {
      margins: {
        x: 0,
        y: -70
      },
  ...
  _.getPopUpLineSettings = function() {
    return this._loader.getParam("popUpLineSettings", {
      text: {
        visible: true,
        size: {
          w: 80,
  ...
  _.getVisualDropItemSettings = function() {
    return this._loader.getParam("visualDropItemSettings", {
      text: {
        visible: true,
        size: {
          w: 80,
  ...
  _.getVisualDropSettings = function() {
    return this._loader.getParam("visualDropMainSettings", {
      autoLootDistanceInPx: 80,
      clickLootDistanceInPx: 160,
      moveSpeed: 5,
      flowAnimation: {
  ...
  _.getItemsRaritySettings = function() {
    return this._loader.getParam("itemsRarityGroup", [
      {
        color: "#a1e065",
        icon: 0
      },
...
(function() {
  ...
  window.EL_ClearAllScreenPopUps = function() {
    var e, i, len, popUp, ref;
    try {
      ref = $gameTemp._pkdPelScreenPopUps;
      for (i = 0, len = ref.length; i < len; i++) {
        popUp = ref[i];
        if ((popUp != null) && popUp.isAlive()) {
          popUp.exitFromScreen();
        }
      }
    } catch (error) {
      e = error;
    ...
    $gameTemp._pkdPelScreenPopUps = [];
    $gameTemp._pkdPelScreenPopUpsQueque = [];
    $gameTemp._pkdPelNeedWaitQueue = false;
  };
  window.EL_ShowScreenPopUpForItem = function() {
    var _exColor, e, itemCount, itemData;
    try {
      itemData = arguments[0];
      itemCount = arguments[1] || 1;
      _exColor = arguments[2];
      PEL_NUI_ScreenPopUpItem.ShowForItem(itemData, itemCount, _exColor);
    } catch (error) {
      e = error;
      KDCore.warning(e);
    }
  };
  window.EL_ShowScreenPopUpCustom = function(iconIndex, text, count, color) {
    var e, fakeItemData;
    try {
      fakeItemData = {
        name: text,
      ...
      return window.EL_ShowScreenPopUpForItem(fakeItemData, count, color);
    } catch (error) {
      e = error;
      return KDCore.warning(e);
    }
...
(function() {
  ...
  _.LoadNetworkCode = function() {
    ...
    return (function() {      //╒═════════════════════════════════════════════════════════════════════════╛
      ...
      //╒═════════════════════════════════════════════════════════════════════════╛
      //---------------------------------------------------------------------------
      var ALIAS__onCustomCommand;
      //@[DEFINES]
      _ = nAPI;
      //@[ALIAS]
      ALIAS__onCustomCommand = _.onCustomCommand;
      _.onCustomCommand = function(name, data) {
        var actorId, e, itemData, itemId, mapId;
        ALIAS__onCustomCommand.call(this, ...arguments);
        try {
          if (name === "pkdExtendedLoot_makeVisualDrop") {
            ({itemData, mapId, actorId} = data);
            ...
            if (KDCore.Utils.isSceneMap()) {
              SceneManager._scene.pMakeVisualDropNowFromNetwork(itemData);
            } else {
              $gameMap.pelAddVisualDropItemData(data);
            }
          } else if (name === "pkdExtendedLoot_removeVisualDrop") {
            ({itemId, mapId} = data);
            if (mapId !== $gameMap.mapId()) {
              return;
            }
            if (KDCore.Utils.isSceneMap()) {
              SceneManager._scene.pRemoveVisualDropFromNetwork(itemId);
            }
            $gameMap.pelRemoveVisualDropItemByUniqueId(itemId);
          }
        } catch (error) {
          e = error;
          KDCore.warning(e);
...
(function() {
  ...
  DataManager._databaseFiles.push({
    name: "$PEL_NUI_ScreenPopUpItem",
    src: "PKD_ExtendedLoot/NUI_ScreenPopUpItem.json"
  });
  //@[ALIAS]
  ALIAS__loadDataFile = _.loadDataFile;
  _.loadDataFile = function(name, src) {
    if (src.contains("PKD_ExtendedLoot")) {
...
(function() {
  ...
  _.pelAddVisualDropItem = function(vdItem) {
    ...
    if (vdItem.isMovingToPlayer()) {
      ...
      this.pelAddVisualDropItemData(vdItem.stored());
    }
  };
  _.pelAddVisualDropItemData = function(data) {
    this.pelInitVisualDropStorage();
    if (data == null) {
      return;
    }
    this.__pelVDS.push(data);
  };
  _.pelRemoveVisualDropItemByUniqueId = function(uniqueId) {
    this.pelInitVisualDropStorage();
    this.__pelVDS = this.__pelVDS.filter(function(vd) {
      return (vd[5] != null) && vd[5] !== uniqueId;
    });
  };
})();
...
(function() {
  ...
  _.pelShowNotifyForItemGain = function(item, amount) {
    ...
    try {
      ...
      if (PKD_ExtendedLoot.PP.isShowOnScreenPopUps()) {
        return this._pelShowNotifyOnScreen(item, amount);
      } else {
        return this._pelShowNotifyAbovePlayer(item, amount);
      }
    } catch (error) {
      e = error;
      return KDCore.warning(e);
  ...
  _._pelShowNotifyAbovePlayer = function(item, amount) {
    ...
    try {
      ...
      if (char != null) {
        char.pelAddTreasurePopUp(popUpItem);
      }
    } catch (error) {
      e = error;
      KDCore.warning(e);
  ...
  _._pelShowNotifyOnScreen = function(item, amount) {
    var e;
    try {
      return window.EL_ShowScreenPopUpForItem(item, amount);
    } catch (error) {
      e = error;
      return KDCore.warning(e);
    }
...
(function() {
  var _;
  // * Общие методы для доступа к классу окна

  //@[DEFINES]
  _ = PKD_ExtendedLoot;
  _.ShowLastGainPopUp = function() {
    return $gameParty.pelOnSomeItemBeenGained();
  };
  _.IsValidGoldItem = function() {
    var itemId;
    itemId = PKD_ExtendedLoot.PP.goldItemId();
    return itemId > 0 && ($dataItems[itemId] != null);
  };
  _.IsGoldItem = function(item) {
    if (!_.IsValidGoldItem()) {
      return false;
    }
    if (item == null) {
      return false;
    }
    return DataManager.isItem(item) && item.id === PKD_ExtendedLoot.PP.goldItemId();
  };
  _.InCollectLootMode = function() {
    return $gameTemp._pCollectForLootWindow === true && KDCore.Utils.isSceneMap();
  };
  _.IsShouldVisualDrop = function() {
    return !this.InCollectLootMode() && !this.IsLootWindowIsOpen() && PKD_ExtendedLoot.PP.isVisualDropActive() && $gameSystem._pelNoVSDrop !== true && KDCore.Utils.isSceneMap();
  };
  _.InitLootListData = function() {
    $gameTemp._pCollectForLootWindow = false;
    $gameTemp._pLootListName = null;
    $gameTemp._pLootListImage = null;
    $gameTemp._pLootListDB = [];
    $gameTemp._pLootEventId = 0;
  };
  _.OpenLootWindow = function() {
    var e;
    try {
      return SceneManager._scene.pOpenLootWindow();
    } catch (error) {
      e = error;
      return KDCore.warning(e);
    }
  };
  _.IsLootWindowIsOpen = function() {
    var e;
    try {
      return SceneManager._scene.pIsLootWindowIsOpen();
    } catch (error) {
      e = error;
      KDCore.warning(e);
      return false;
  ...
  _.CloseLootWindow = function() {
    var e;
    try {
      return SceneManager._scene.pCloseLootWindow();
    } catch (error) {
      e = error;
      return KDCore.warning(e);
    }
  };
  _.GetItemRarityData = function(item) {
    var e, levels, rarity;
    if (!PKD_ExtendedLoot.PP.isRaritySystemActive()) {
      return null;
    }
    if (item == null) {
      return null;
    ...
    try {
      if ((item.meta != null) && (item.meta.pRarity != null)) {
        rarity = parseInt(item.meta.pRarity);
        if (rarity >= 1) { // * С 1-цы, так как параметры плагина с 1 - цы написаны
          // * А сам массив с нуля начинается
          levels = PKD_ExtendedLoot.PP.getItemsRaritySettings();
...
// Generated by CoffeeScript 2.6.1
var PEL_NUI_ScreenPopUpItem;

PEL_NUI_ScreenPopUpItem = class PEL_NUI_ScreenPopUpItem extends KDCore.Sprite {
  constructor(itemData1, itemCount1, _exColor1) {
    super();
    this.itemData = itemData1;
    this.itemCount = itemCount1;
    this._exColor = _exColor1;
    this._prepareItem();
    this._create();
    this._onAppears();
    this._aliveTimer = 0;
    this._stayTimer = this.uiConstant('stayAliveTimeSeconds') * 60;
    return;
  }

  static ShowForItem(itemData, itemCount, _exColor) {
    var i, lastPopUp, len, popUp, ref;
    if ($gameTemp._pkdPelScreenPopUps == null) {
      $gameTemp._pkdPelScreenPopUps = [];
    }
    if (itemData == null) {
      return;
    }
    if ($gameTemp._pkdPelScreenPopUpsQueque == null) {
      $gameTemp._pkdPelScreenPopUpsQueque = [];
    }
    if ($gameTemp._pkdPelNeedWaitQueue === true) {
      $gameTemp._pkdPelScreenPopUpsQueque.push([itemData, itemCount, _exColor]);
      return;
    }
    if (PKD_ExtendedLoot.PP.isRiseCountForSameItem() === true) {
      // * Check if last PopUp ItemData is same?
      lastPopUp = $gameTemp._pkdPelScreenPopUps.last();
      if ((lastPopUp != null) && lastPopUp.isAlive() && lastPopUp.itemData === itemData) {
        lastPopUp.riseCount(itemCount);
        return lastPopUp;
      }
    }
    ref = $gameTemp._pkdPelScreenPopUps;
    // * Check if we have PopUp with same ItemData
    /*for popUp in $gameTemp._pkdPelScreenPopUps
    if popUp? and popUp.isAlive() && popUp.itemData == itemData
        popUp.riseCount(itemCount)
        return popUp*/
    for (i = 0, len = ref.length; i < len; i++) {
      popUp = ref[i];
      if ((popUp != null) && popUp.isAlive()) {
    ...
    $gameTemp._pkdPelNeedWaitQueue = true;
    setTimeout((function() {
      var e;
      try {
        // * Create new PopUp
        ...
        popUp = new PEL_NUI_ScreenPopUpItem(itemData, itemCount, _exColor);
        SceneManager._scene.addChild(popUp);
        return $gameTemp._pkdPelScreenPopUps.push(popUp);
      } catch (error) {
        e = error;
        return KDCore.warning(e);
      }
    ...
    setTimeout((function() {
      return $gameTemp._pkdPelNeedWaitQueue = false;
    }), PKD_ExtendedLoot.PP.screenPopUpQueueDelayInMs());
    return popUp;
  }
  ...
  update() {
    ...
    try {
      this._aliveTimer += 1;
      if (this._aliveTimer > this._stayTimer) {
        this._aliveTimer = -1;
        this.exitFromScreen();
      }
    } catch (error) {
      e = error;
      KDCore.warning(e);
      this._aliveTimer = -1; // * Disable Update
    }
  }
  ...
  _prepareItem() {
    var rarityData;
    this._exCount = null;
    this._exIconIndex = null;
    this._rarityIconIndex = null;
    if (PKD_ExtendedLoot.IsGoldItem(this.itemData)) {
      this._exColor = PKD_ExtendedLoot.PP.goldItemTextColor();
    } else {
      rarityData = PKD_ExtendedLoot.GetItemRarityData(this.itemData);
      if (rarityData != null) {
        this._exColor = rarityData.color;
        this._rarityIconIndex = rarityData.icon;
      }
    }
  }
  ...
  _onAppears() {
    var e;
    try {
      return KDCore.Utils.playSE(this.uiConstant('itemAppearsSE'));
    } catch (error) {
      e = error;
      return KDCore.warning(e);
    }
  ...
  riseCount(value) {
    if (this._exCount == null) {
      this._exCount = this.count();
    }
    this._aliveTimer = 0;
    this._exCount = this._exCount + value;
    return this.refreshBindings(this);
  }

  isAlive() {
    return this._aliveTimer >= 0 && (this.parent != null);
  }

  isHaveIcon() {
    var e;
    try {
      if (this._exIconIndex != null) {
        return this._exIconIndex > 0;
      }
      return (this.itemData != null) && this.itemData.iconIndex > 0;
    } catch (error) {
      e = error;
      KDCore.warning(e);
    }
  ...
  iconIndex() {
    ...
    try {
      if (this._exIconIndex != null) {
        return this._exIconIndex;
      }
      if (this.itemData != null) {
        return this.itemData.iconIndex;
      }
    } catch (error) {
      e = error;
      KDCore.warning(e);
  ...
  isHaveRarityIcon() {
    return (this._rarityIconIndex != null) && this._rarityIconIndex > 0;
  }

  rareIconIndex() {
    return this._rarityIconIndex || 0;
  }
  ...
  moveNext() {
    var animation, e, i, len, moveNextAnimations, results, rule;
    try {
      moveNextAnimations = this.uiConstant('moveNextAnimations');
      if ((moveNextAnimations != null) && moveNextAnimations.length > 0) {
        results = [];
        for (i = 0, len = moveNextAnimations.length; i < len; i++) {
          animation = moveNextAnimations[i];
          rule = new KDCore.AnimationRule(animation, this);
          results.push(this.addAnimationRule(rule));
        }
        return results;
      }
    } catch (error) {
  ...
  exitFromScreen() {
    var animation, e, exitAnimations, i, index, len, rule;
    try {
      KDCore.Utils.playSE(this.uiConstant('itemExitSE'));
      exitAnimations = this.uiConstant('exitAnimations');
      if ((exitAnimations != null) && exitAnimations.length > 0) {
        for (index = i = 0, len = exitAnimations.length; i < len; index = ++i) {
          animation = exitAnimations[index];
          rule = new KDCore.AnimationRule(animation, this);
          if (index === 0) {
            rule.setEndCallback(this.removeFromParent.bind(this));
          }
          this.addAnimationRule(rule);
        }
      } else {
        this.removeFromParent();
      }
  ...
  _create() {
    KDCore.Sprite_NUI.FromScheme($PEL_NUI_ScreenPopUpItem, this);
    return this.refreshBindings(this);
  }
...
(function() {  
  ...
  _._linkMe = function() {
    ...
    try {
      ...
      if ((ref = SceneManager._scene._spriteset._pelVisualDropItemsBase) != null) {
        ref.addChild(this);
      }
      this.move(this.parentCharacter.x + this.params.margins.x, this.parentCharacter.y + this.params.margins.y);
    } catch (error) {
      e = error;
      KDCore.warning(e);
    }
...
VisualDropItem = class VisualDropItem extends KDCore.Sprite {
  ...
  playDropSound() {
    var item;
    item = this.item();
    if ((item != null) && (item.meta != null) && String.any(item.meta.lootDropSound)) {
      KDCore.Utils.playSE(item.meta.lootDropSound);
    } else {
  ...
  stored() {
    return [this.mx, this.my, this.itemType, this.itemId, this.amount, this.uniqueId];
  }
  ...
  dispose() {
    ...
    if (this.isHaveUniqueId()) {
      this.disposeForNetwork();
    }
  }
  ...
  disposeForNetwork() {
    var e, networkData;
    try {
      if (PKD_ExtendedLoot.PP.isNetworkGlobalVisualDrop()) {
        networkData = {
          itemId: this.uniqueId,
          mapId: $gameMap.mapId()
        };
        return nAPI.sendCustomCommand("pkdExtendedLoot_removeVisualDrop", networkData);
      }
    } catch (error) {
      e = error;
      return KDCore.warning(e);
  ...
  isInAutoLootRadius() {
    var dist;
    if (this.isItemHaveNoAutoLootNoteTag()) {
      return false;
    }
...
//---------------------------------------------------------------------------
(function() {
  var ALIAS__createSpriteset, ALIAS__isAnyButtonPressed, ALIAS__onMapLoaded, ALIAS__processMapTouch, ALIAS__stop, ALIAS__update, _;
  //@[DEFINES]
  _ = Scene_Map.prototype;
  //@[ALIAS]
  ...
  ALIAS__stop = _.stop;
  _.stop = function() {
    if (!PKD_ExtendedLoot.PP.isContinueShowPopUpsAfterMapChange()) {
      $gameTemp._pkdPelScreenPopUpsQueque = [];
    }
    $gameTemp._pkdPelScreenPopUps = [];
    $gameTemp._pkdPelNeedWaitQueue = false;
  ...
  _.update = function() {
    ...
    try {
      ...
      if ($gameTemp._pkdPelScreenPopUpsQueque.length > 0 && !$gameTemp._pkdPelNeedWaitQueue) {
        data = $gameTemp._pkdPelScreenPopUpsQueque.shift();
        EL_ShowScreenPopUpForItem(...data);
      }
    } catch (error) {
      e = error;
      return KDCore.warning(e);
...
(function() {
  ...
  _.pMakeVisualDropNow = function(data, withJump = true) {
    ...
    try {
      ...
      if (data[5] != null) {
        dropItem.setUniqueId(data[5]);
      }
      this._pelVisualDropSpriteset.addChild(dropItem);
      try {
        if (PKD_ExtendedLoot.PP.isNetworkGlobalVisualDrop()) {
          if (!dropItem.isHaveUniqueId()) { // * New Item (only spaw)
            dropItem.setUniqueId(KDCore.makeId(40));
            data[5] = dropItem.uniqueId;
            dataForNetwork = {
              itemData: data,
              mapId: $gameMap.mapId(),
              actorId: $gameParty.leader().actorId()
            };
            nAPI.sendCustomCommand("pkdExtendedLoot_makeVisualDrop", dataForNetwork);
          }
        }
      } catch (error) {
        e = error;
  ...
  _.pMakeVisualDropNowFromNetwork = function(data) {
    var e;
    try {
      if ((data != null) && (data[5] != null)) {
        // * Only if Item alredy have uniqueId
        return this.pMakeVisualDropNow(data);
      }
    } catch (error) {
      e = error;
      return KDCore.warning(e);
  ...
  _.pRemoveVisualDropFromNetwork = function(uniqueId) {
    var e, sprite;
    try {
      if (this._pelVisualDropSpriteset == null) {
        return;
      }
      sprite = this._pelVisualDropSpriteset.children.find(function(i) {
        return i.isHaveUniqueId() && i.uniqueId === uniqueId;
      });
      if (sprite != null) {
        this._pelVisualDropSpriteset.removeChild(sprite);
      }
    } catch (error) {
      e = error;
      KDCore.warning(e);
...
(function() {
  ...
  _.createDestination = function() {
    ...
    this._pelVisualDropItemsBase.z = 9;
...
//Plugin PKD_ExtendedLoot builded by PKD PluginBuilder 2.2.1 - 24.06.2024